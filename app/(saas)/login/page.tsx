...
import { loginAction } from "@/app/(saas)/actions";
  ...
  return (
    <div style={{
      ...
      <p style={{ color: "var(--muted)", margin: "0 0 2rem", fontSize: "0.9rem" }}>
        Sign in to your Mirha & Co. account
      </p>
      ...
      <p style={{ margin: "0.8rem 0 0", fontSize: "0.85rem", color: "var(--muted)" }}>
        Don't have an account? <Link href={`/register?redirect=${encodeURIComponent(redirectVal)}`} style={{ color: "var(--rose)", textDecoration: "none", fontWeight: 600 }}>Sign up</Link>
      </p>
    </div>
  );
}
