...
export default function SiteHeader() {
  ...
  return (
    <header className="site-header relative z-[200] border-b border-[#ded7cf] bg-white transition-colors duration-300 dark:border-white/10 dark:bg-[#121110]">
      <div className="site-header-container">
        ...
        <div className="site-header-left">
          <nav className="site-header-nav">
            ...
            <Link href="/tools/hard-water"  className="site-header-nav-link nav-hide-1024">{t("nav.hardwater")}</Link>
            <Link href="/tools/dupes"       className="site-header-nav-link nav-hide-1100">{t("nav.dupes")}</Link>
            <Link href="/k-beauty"          className="site-header-nav-link site-header-nav-link--accent nav-hide-1280">{t("nav.kbeauty")}</Link>
            <Link href="/mens-grooming"     className="site-header-nav-link nav-hide-1350">{t("nav.men")}</Link>
            <Link href="/blog"              className="site-header-nav-link nav-hide-1400">{t("nav.blog")}</Link>
          </nav>
        </div>
        ...
        <div className="site-header-right">
          <nav className="site-header-nav">
            <Link href="/b2b"       className="site-header-nav-link nav-hide-1400">{t("nav.b2b")}</Link>
            <Link href="/about"     className="site-header-nav-link nav-hide-1100">{t("nav.about")}</Link>
            <Link href="/pricing"   className="site-header-nav-link nav-hide-1280">{t("nav.pricing")}</Link>
            <Link href="/dashboard" className="site-header-nav-link nav-hide-1350">{t("nav.dashboard")}</Link>
      ...
      {/* Mobile & Full Navigation Drawer (Sleek & Pinned Controls) */}
      {menuOpen && (
        <div
        ...
        >
          ...
          {/* Drawer panel */}
          <div
            className="site-header-drawer-content flex h-full w-[340px] sm:w-[380px] max-w-[88vw] flex-col justify-between border-r border-[#ded7cf] bg-[#fcfbf9] p-6 shadow-2xl transition-colors duration-300 dark:border-white/15 dark:bg-[#141312] dark:text-[#f7f5f2]"
            style={{ zIndex: 100000, overflowY: "auto" }}
          >
            {/* Top Container: Header & Nav Links */}
            <div className="flex flex-col gap-6">
              {/* Drawer Header */}
              <div className="flex items-center justify-between border-b border-[#e5ded6] pb-4 dark:border-white/10">
                <span className="font-bebas notranslate text-2xl tracking-[0.1em] text-[#11100f] dark:text-white">
                  MIRHA &amp; CO.
                </span>
                <button
                  onClick={() => setMenuOpen(false)}
              ...
              <div className="flex flex-col gap-1">
                <div className="mb-2 text-[11px] font-extrabold uppercase tracking-[0.15em] text-[#fc2779] dark:text-[#ff4d94]">
                  Tools &amp; Intelligence
                </div>
                <Link
            ...
            {/* Bottom Section: Unified Control Toolbar (Anchored cleanly at bottom) */}
            <div className="mt-8 border-t border-[#e5ded6] pt-4 dark:border-white/10">
