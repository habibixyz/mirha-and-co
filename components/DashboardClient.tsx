...
export function DashboardClient({ user, routines, recentJournal, stats }: any) {
 ...
 if (hour >= 17) setGreeting("Good Evening");
  ...
  useEffect(() => {
    if (routines && routines.length > 0) {
      ...
      routines.forEach((r: any) => {
        newMap[r.id] = r.metadata?.logs?.[today] || [];
      });
      setCheckedStepsMap(newMap);
    } else {
      setCheckedStepsMap({});
    }
  }, [routines, today]);

  const toggleStep = async (routineId: string, index: number) => {
    if (!routineId) return;

    const currentChecked = checkedStepsMap[routineId] || [];
    const isChecked = currentChecked.includes(index);
  ...
  const routinesToDisplay = routines && routines.length > 0 
    ? routines 
    : [
        {
          id: null,
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        {/* Routines */}
        {routinesToDisplay.map((routine: any, rIdx: number) => {
                ...
                {steps.map((step: string, idx: number) => {
                  ...
                  return (
                    <motion.div
                      ...
                      whileTap={{ scale: 0.99 }}
                      onClick={() => routineId ? toggleStep(routineId, idx) : null}
                      style={{
                        display: "flex",
                        alignItems: "center",
                        gap: "0.8rem",
